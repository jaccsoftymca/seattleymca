...
use Drupal\node\NodeInterface;
use Drupal\ygs_session_instance\Entity\SessionInstance;
...
class SessionInstanceManager implements SessionInstanceManagerInterface {
   ...
   */
  const CHANNEL = 'ygs_session_instance';
  ...
  public function resetCache() {
    ...
    \Drupal::moduleHandler()->invokeAll('ygs_session_instance_reset_cache');
  }
  ...
  public function getAgeIds(NodeInterface $session) {
    ...
    if (!empty($terms)) {
      foreach ($terms as $id => $term) {
        ...
        $ag_max = !empty($term->field_session_max_age->value) ? $term->field_session_max_age->value : 0;
  ...
  public function getSessionData(NodeInterface $session) {
    $content_moderation_info = \Drupal::service(ygs_moderation_wrapper_active_module() . '.moderation_information');

    // Skip session with empty location reference or closed sales status.
    if (empty($session->field_session_location->target_id) || $session->field_sales_status->value == 'close') {
      return NULL;
    }
    $location_id = $session->field_session_location->target_id;

    $facility_id = NULL;
    if (!empty($session->field_session_plocation->target_id)) {
      $facility_id = $session->field_session_plocation->target_id;
    }
    ...
    $class_id = $class->id();
    $class_type = $class->field_type->value;
    ...
    // Age reference.
    $age_ids = $this->getAgeIds($session);
    ...
    return [
      ...
      'field_activity' => array_unique($activity_ids),
      'field_program_subcategory' => array_unique($program_subcategory_ids),
      'field_program' => array_unique($program_ids),
      ...
      if ($schedule_item['frequency'] == 'weekly' || $schedule_item['frequency'] == 'daily') {
        ...
        while ($candidate_day <= $end_timestamp) {
          ...
          elseif ($candidate_day < $timestamp) {
            ...
            if (!$excluded) {
              $session_instances[] = [
                'actual' => (bool) $schedule_item['actual'],
  ...
  public static function loadSessionSchedule(NodeInterface $node) {
    ...
    foreach ($dates as $date) {
      $schedule_item = [
        'actual' => $date->field_session_time_actual->getValue()[0]['value'],
