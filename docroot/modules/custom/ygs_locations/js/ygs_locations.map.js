(function ($, window, Drupal, drupalSettings) {
  ...
  Drupal.ygs_locations_map = function() {
    return {
      ...
      // Executed every time a chosen select filter state changes.
      ...
      apply_autocomplete_search: function (event, ui) {
        ...
        for (l = 0; l < this.locations.length; l++) {
          ...
          $(this.locations[l].element).parents('.locations-list').find('h2:not(.h1)').hide();
        }
        if (!locations.length) {
        ...
        for (l = 0; l < locations.length; l++) {
          ...
          $(locations[l].element).parents('.locations-list').find('h2:not(.h1)').show();
        }
      },
      ...
      // Applies the current select state of the tag filter controls
      // to the internal filters data structure.
      // Called at init time, and after every select state change.
      update_tag_filters: function () {
        var active_filters = $('select#tag-filter').val();
        if (active_filters) {
          this.tag_filters = active_filters;
        }
        else {
          // If tags filter empty - set all available tags.
          this.tag_filters = [];
      ...
      set_url_parameters: function () {
        ...
        var filters_tags_raw = $('select#tag-filter').val();
        var filters_tags = '';
        if (filters_tags_raw) {
          filters_tags = '?type=';
      ...
      draw_map_controls: function () {
        ...
        var tag_filters_html = '<select id="tag-filter" class="form-control multiselect" name="tag_filter" multiple="multiple">';
        ...
        var url_parameters = this.get_parameters();
        if (url_parameters.distance_limit) {
          // Set distance_limit from url if defined.
          this.distance_limit_el.val(url_parameters.distance_limit);
          this.distance_limit = url_parameters.distance_limit;
        }
      },
  ...
  Drupal.behaviors.ygs_locations_map = {};
  Drupal.behaviors.ygs_locations_map.attach = function (context) {
    $(context).find('body').once('ygs-locations').each(function () {
      // Get locations from HTML.
      var locations = {
        locations: []
      };
      $('.locations-list .node--view-mode-teaser').each(function() {
        var $self = $(this), data = $self.data('ygs-locations');
        data.element = $self.parent();
        data.markup = $self.get(0).outerHTML;
        locations.locations.push(data);
      });

      var map = new Drupal.ygs_locations_map();
      map.init({
        component_el: $('#ygs-locations-map'),
        map_data: locations
      });
    });
  };
