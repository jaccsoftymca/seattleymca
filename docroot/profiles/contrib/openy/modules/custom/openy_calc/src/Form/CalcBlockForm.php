...
class CalcBlockForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // Check for 'membership_types' args.
    $args = $form_state->getBuildInfo()['args'];
    $membership_types = FALSE;
    $membership_types_negate = FALSE;
    if (!empty($args)) {
      foreach ($args as $v) {
        if (isset($v['membership_types'])) {
          $membership_types = $v['membership_types'];
        }
        if (isset($v['membership_types_negate']) && $v['membership_types_negate']) {
          $membership_types_negate = TRUE;
        }
      }
    }
    $step = 1;
    ...
    $types = $this->dataWrapper->getMembershipTypes([
      ...
      'membership_types_negate' => $membership_types_negate,
    ]);
