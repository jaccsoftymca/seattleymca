...
PHPCS_RESULT="$(phpcs --standard=Drupal --extensions=php,module,inc,install,test,profile,theme,css,info,txt,md docroot/themes/custom docroot/modules/custom)"
...
if echo "$PHPCS_RESULT" | grep -q "$STR_ERROR"; then
  ...
  PHPCS_CSV="$(phpcs --standard=Drupal --extensions=php,module,inc,install,test,profile,theme,css,info,txt,md docroot/themes/custom docroot/modules/custom --report=csv)"
  CURRENT_DIR=$(pwd)
